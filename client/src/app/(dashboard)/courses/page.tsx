...
export default function CoursesDashboardPage() {
  ...
  return (
    <div
      className="mx-auto flex w-full max-w-md flex-col overflow-hidden rounded-none bg-white shadow-xl sm:rounded-3xl"
      style={{ height: '100svh' }}
    >
      {/* Zone 1: Header — always top, never scrolls */}
      <div className="shrink-0 flex min-w-0 items-center justify-between border-b border-slate-100 px-4 py-4">
      ...
      {/* Zone 3: Sync button — always bottom, never scrolls, hidden when synced */}
      {!syncResult && (
        <div className="shrink-0 border-t border-slate-100 px-4 py-4">
          {isApple ? (
            ...
            >
              ...
              {isSyncing ? 'Đang xuất lịch...' : 'Thêm vào Apple Calendar'}
            </button>
          ) : (
            <button
              type="button"
              onClick={handleSync}
              disabled={courses.length === 0 || isSyncing}
              className="flex h-12 w-full items-center justify-center rounded-xl bg-slate-900 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
            >
